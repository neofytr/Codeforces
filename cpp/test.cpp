...
2. struct uses public inheritance by default; class uses private inheritance by
default
...
Scope -> enumerator names are injected into the score surrounding the
declaration Enumerators can be implicitly converted to int and compared with
integers
...
that it's enumerators are scoped. Initializing an enum with a value from it's
underlying type (by default, thats int) is allowed -> Color x = Color{1}; Color
y {1};

By default, enum class has assignment, initialization, and comparisons defined,
and only those. However, an enumeration is a user-defined type, so we can define
operators for it ->
...
/* Here, v can hold either a double, or an int, but not both */
/* Unlike unions, the compiler enforces correctness: */
/* you can ask holds_alternative<int>(v) -> is it an int? */
